...
logger = logging.getLogger("gold.dim_field")
...
src_sql = '''
CREATE OR REPLACE TABLE gold.dim_field AS
SELECT DISTINCT
...
cfg = {
    'TARGET_TABLE': 'gold.dim_field',
    'SOURCE_SQL': src_sql,
    'MERGE_KEYS': ['FieldId_key'],
    'DELETE_KEYS': [],
    'load_strategy': 'silver_to_gold',
}

spark.sql(src_sql)
logger.info(f"[DIRECT] Created gold.dim_field ({spark.table('gold.dim_field').count()} rows)")
