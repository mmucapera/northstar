...
logger = logging.getLogger("gold.fact_hse_exposure")
...
src_sql = '''
CREATE OR REPLACE TABLE gold.fact_hse_exposure AS
SELECT DISTINCT
CAST(xxhash64(PeriodId) AS BIGINT) AS PeriodId_key,
T.HoursWorked,
T.Manifest_package,
T.Manifest_file
FROM silver.hse_exposure T
'''

cfg = {
    'TARGET_TABLE': 'gold.fact_hse_exposure',
    'SOURCE_SQL': src_sql,
    'MERGE_KEYS': ['PeriodId_key'],
    'DELETE_KEYS': [],
    'load_strategy': 'silver_to_gold',
}

spark.sql(src_sql)
logger.info(f"[DIRECT] Created gold.fact_hse_exposure ({spark.table('gold.fact_hse_exposure').count()} rows)")
