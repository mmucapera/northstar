...
logger = logging.getLogger("gold.fact_downtime")
...
src_sql = '''
CREATE OR REPLACE TABLE gold.fact_downtime AS
SELECT DISTINCT
...
T.Manifest_package,
T.Manifest_file
FROM silver.downtime T
'''

cfg = {
    'TARGET_TABLE': 'gold.fact_downtime',
    'SOURCE_SQL': src_sql,
    'MERGE_KEYS': ['PeriodId_key', 'CauseId_key'],
    'DELETE_KEYS': [],
    'load_strategy': 'silver_to_gold',
}

spark.sql(src_sql)
logger.info(f"[DIRECT] Created gold.fact_downtime ({spark.table('gold.fact_downtime').count()} rows)")
