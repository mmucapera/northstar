...
logger = logging.getLogger("gold.fact_production_volume")
...
src_sql = '''
CREATE OR REPLACE TABLE gold.fact_production_volume AS
SELECT DISTINCT
...
T.Manifest_package,
T.Manifest_file
FROM silver.production_volume T
'''

cfg = {
    'TARGET_TABLE': 'gold.fact_production_volume',
    'SOURCE_SQL': src_sql,
    'MERGE_KEYS': ['PeriodId_key', 'WellId_key'],
    'DELETE_KEYS': [],
    'load_strategy': 'silver_to_gold',
}

spark.sql(src_sql)
logger.info(f"[DIRECT] Created gold.fact_production_volume ({spark.table('gold.fact_production_volume').count()} rows)")
...
tbl = 'gold.fact_production_volume'
