...
# # nb_config_delete_insert_manifest
# > Loaded by `nb_utils_config`. Do NOT %run this notebook directly.
# > Contents: delete_insert_manifest
...
# ## Delete-Insert with Manifest (v2)
# Optimized manifest-driven partial loading with batched `IN` / `RLIKE` clauses and consolidated delete key handling.
...
def delete_insert_manifest(cfg):
    ...
    for _attempt in range(1, _max_retries + 1):
        try:
            ...
            # Insert new rows
            df.write.mode("append").option("mergeSchema", "true").format("delta").saveAsTable(tgt)
...
# [LOAD_CONFIRM]
print("[CONFIG] loaded nb_config_delete_insert_manifest")
